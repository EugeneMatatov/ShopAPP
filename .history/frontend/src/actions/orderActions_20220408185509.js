...
export const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    order.orderItems.map((i) => {
      const temp = i.img.split(",")[1];
      //console.log("before convertion");
      //console.log(temp);
      orderItemsM.push({
        ...i,
        img: { data: { type: "Buffer", data: [temp] } },
      });
      //console.log("after convertion");
      // console.log(base64ToArrayBuffer(temp));
    });
    order = { ...order, orderItems: orderItemsM };
    //console.log("order befor sending to server");
    //console.log(order);
    const {
      userLogin: { userInfo },
    } = getState();
